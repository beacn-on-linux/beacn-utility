                        ...
                        if comp.mode == Simple {
                            ui.horizontal_centered(|ui| {
                                ...
                                if s.changed() {
                                    ...
                                    let message =
                                        Message::Compressor(Compressor::Ratio(Simple, ratio));
                            ...
                            if s.changed() {
                                let ratio = CompressorRatio(values.ratio);
                                let message =
                                    Message::Compressor(Compressor::Ratio(Advanced, ratio));
                                state.handle_message(message).expect("Failed");
                            }
                            ...
                            if s.changed() {
                                ...
                                let message =
                                    Message::Compressor(Compressor::Attack(Advanced, attack));
                                state.handle_message(message).expect("Failed");
                            }
